        ...
        infile='../data/F2_Dp_GLOBAL_DATA.out'
        write(6,*) infile

        OPEN(UNIT=7,FILE=infile)
        READ(7,'(A72)') COMMENT

        outfile='OUT/F2Dp.out'
        open(unit=66,file=outfile)
        write(66,*) 'exp   x    Q2    WSQ   F2Dp_data   F2Dp_err  F2Dp_norm  ',
     >     'F2Dp_f1f217   F2Dp_ineft   F2d_f1f217   F2p_f1f217',
     >     'F2d_ineft   F2p_ineft'
        ...
        do 99 ii=1,100000
           x=0.0
           Q2=0.0
           READ(7,'(F5.1,F7.4,F9.3,3F10.4,F12.5,F12.6)',END=100) nexp,x,Q2,F2data,
     >     f2_err,f2_norm
           IF (x.LE.0.) goto 100
           IF (q2.gE.14.) goto 99
           ...
           write(66,'(F3.1,3f10.5,3F13.5,2F17.5,4F17.5)')nexp,x,q2,wsq,f2data,f2_err,f2_norm,f2Dp_f1f217,f2Dp_ineft,
