...
void plot_F2Dp()
{
     ...
     Double_t F2Dp[MAXBIN]={0.0},F2Dp_err[MAXBIN]={0.0},F2Dp_norm[MAXBIN]={0.0};
     Double_t F2Dp_f1f217[MAXBIN]={0.0},F2Dp_ineft[MAXBIN]={0.0};
     Double_t F2d_f1f217[MAXBIN]={0.0},F2p_f1f217[MAXBIN]={0.0};
     Double_t F2d_ineft[MAXBIN]={0.0},F2p_ineft[MAXBIN]={0.0};
     Double_t nexp[MAXBIN]={0.0};
     Ssiz_t from=0;
     TString content,tmp;
     int nn=0;
     while(tmp.ReadLine(file1)){
	   ...
	   nexp[nn-1]=atof(content.Data());
           ...
           tmp.Tokenize(content,from," ");
           F2Dp[nn-1]=atof(content.Data());
           tmp.Tokenize(content,from," ");
           F2Dp_err[nn-1]=atof(content.Data());
           tmp.Tokenize(content,from," ");
           F2Dp_norm[nn-1]=atof(content.Data());
           tmp.Tokenize(content,from," ");
           F2Dp_f1f217[nn-1]=atof(content.Data())/2.0;
           tmp.Tokenize(content,from," ");
           F2Dp_ineft[nn-1]=atof(content.Data());
           tmp.Tokenize(content,from," ");
           F2d_f1f217[nn-1]=atof(content.Data());
           tmp.Tokenize(content,from," ");
	   ...
	   F2Dp_err[nn-1]=F2Dp[nn-1]*sqrt(F2Dp_err[nn-1]*F2Dp_err[nn-1]+F2Dp_norm[nn-1]*F2Dp_norm[nn-1]);
           nn++;
           from=0;
         }
     file1.close();
     ...
     ofstream myfile;
     myfile.open("Bad_f2Dp.out");
     myfile<<"exp   x   Q2   W2   F2d/p    F2d/p_err   F2d/p_f1f217    f2d/p_ineft    F2d_f1f217    F2p_f1f217    F2d_ineft    F2p_ineft"<<endl;
     int np1=0,np2=0,np3=0,np4=0;
     for(int ii=0;ii<MAXBIN;ii++){
	 if(Q2[ii]>=14||W2[ii]>=14)continue;
	 Double_t tmp_r=F2Dp[ii]/F2Dp_f1f217[ii];
         Double_t tmp_r1=F2Dp[ii]/F2Dp_ineft[ii];
         if(abs(tmp_r-1.0)>0.2 || abs(tmp_r1-1.0)>0.2)
	    myfile<<nexp[ii]<<"  "<<xbj[ii]<<"  "<<Q2[ii]<<"  "<<W2[ii]<<"  "<<F2Dp[ii]<<"  "<<F2Dp_err[ii]<<"  "<<F2Dp_f1f217[ii]<<"  "<<F2Dp_ineft[ii]<<"  "<<F2d_f1f217[ii]<<"  "<<F2p_f1f217[ii]<<"  "<<F2d_ineft[ii]<<"  "<<F2p_ineft[ii]<<endl;
         if(Q2[ii]<8){
            gData[0]->SetPoint(np1,xbj[ii],F2Dp[ii]);
            gData[0]->SetPointError(np1,0,F2Dp_err[ii]);
            gE[0]->SetPoint(np1,xbj[ii],F2Dp_f1f217[ii]);
            gB[0]->SetPoint(np1,xbj[ii],F2Dp_ineft[ii]);
            gDE[0]->SetPoint(np1,xbj[ii],tmp_r);
            gDE[0]->SetPointError(np1,0,F2Dp_err[ii]/F2Dp_f1f217[ii]);
            gDB[0]->SetPoint(np1,xbj[ii],tmp_r1);
            gDB[0]->SetPointError(np1,0,F2Dp_err[ii]/F2Dp_ineft[ii]);
	    np1++;
         }
         if(Q2[ii]<10 && Q2[ii]>=8){
            gData[1]->SetPoint(np2,xbj[ii],F2Dp[ii]);
            gData[1]->SetPointError(np2,0,F2Dp_err[ii]);
            gE[1]->SetPoint(np2,xbj[ii],F2Dp_f1f217[ii]);
            gB[1]->SetPoint(np2,xbj[ii],F2Dp_ineft[ii]);
            gDE[1]->SetPoint(np2,xbj[ii],tmp_r);
            gDE[1]->SetPointError(np2,0,F2Dp_err[ii]/F2Dp_f1f217[ii]);
            gDB[1]->SetPoint(np2,xbj[ii],tmp_r1);
            gDB[1]->SetPointError(np2,0,F2Dp_err[ii]/F2Dp_ineft[ii]);
	    np2++;
         }
         if(Q2[ii]<12 && Q2[ii]>=10){
            gData[2]->SetPoint(np3,xbj[ii],F2Dp[ii]);
            gData[2]->SetPointError(np3,0,F2Dp_err[ii]);
            gE[2]->SetPoint(np3,xbj[ii],F2Dp_f1f217[ii]);
            gB[2]->SetPoint(np3,xbj[ii],F2Dp_ineft[ii]);
            gDE[2]->SetPoint(np3,xbj[ii],tmp_r);
            gDE[2]->SetPointError(np3,0,F2Dp_err[ii]/F2Dp_f1f217[ii]);
            gDB[2]->SetPoint(np3,xbj[ii],tmp_r1);
            gDB[2]->SetPointError(np3,0,F2Dp_err[ii]/F2Dp_ineft[ii]);
	    np3++;
         }
         if(Q2[ii]<14 && Q2[ii]>=12){
            gData[3]->SetPoint(np4,xbj[ii],F2Dp[ii]);
            gData[3]->SetPointError(np4,0,F2Dp_err[ii]);
            gE[3]->SetPoint(np4,xbj[ii],F2Dp_f1f217[ii]);
            gB[3]->SetPoint(np4,xbj[ii],F2Dp_ineft[ii]);
            gDE[3]->SetPoint(np4,xbj[ii],tmp_r);
            gDE[3]->SetPointError(np4,0,F2Dp_err[ii]/F2Dp_f1f217[ii]);
            gDB[3]->SetPoint(np4,xbj[ii],tmp_r1);
            gDB[3]->SetPointError(np4,0,F2Dp_err[ii]/F2Dp_ineft[ii]);
 	    np4++;
         }
     }
     myfile.close();
     ...
     mg2->GetYaxis()->SetRangeUser(0.68,1.32);
     ...
     mg3->GetYaxis()->SetRangeUser(0.68,1.32);
     mg3->GetYaxis()->SetLabelSize(0.1);
     mg3->GetYaxis()->SetNdivisions(8);
     ...
     mg4->GetYaxis()->SetRangeUser(0.68,1.32);
     mg4->GetYaxis()->SetLabelSize(0.1);
     mg4->GetYaxis()->SetNdivisions(8);
