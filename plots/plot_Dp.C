...
void plot_Dp()
{
     ...
     Double_t D2_x[4][MAXBIN],D2_Q2[4][MAXBIN],D2_Yield[4][MAXBIN],D2_Yerr[4][MAXBIN];
     Double_t D2R_x[4][MAXBIN],D2R_Q2[4][MAXBIN],D2_Born1[4][MAXBIN],D2_Rad1[4][MAXBIN];
     Double_t D2_Born2[4][MAXBIN],D2_Rad2[4][MAXBIN];
     Double_t Dp_ratio[4][MAXBIN],Dp_err[4][MAXBIN],D2_RC1[4][MAXBIN],H1_RC1[4][MAXBIN],D2_RC2[4][MAXBIN],H1_RC2[4][MAXBIN];
  ...
  TGraphErrors *gDpRC1[4];
  TGraphErrors *gDpRC2[4];
  TGraphErrors *gRatio[4];
  for(int ii=0;ii<4;ii++){
      ...
      for(int jj=0;jj<MAXBIN;jj++){
          ...
          gDpRC1[ii]->SetPoint(nn1,D2_x[ii][jj],ratio1);
          gDpRC1[ii]->SetPointError(nn1,0.0,ratio1_err);
          gDpRC2[ii]->SetPoint(nn1,D2_x[ii][jj],ratio2);
  ...
  for(int ii=0;ii<4;ii++){
      leg1->AddEntry(gDpRaw[ii],Form("D/p kin%d",ii+1),"P");
  }
  leg1->Draw();
  ...
  for(int ii=0;ii<4;ii++){
      leg2->AddEntry(gDpRC1[ii],Form("gsmearing kin%d",ii+1),"P");
      leg2->AddEntry(gDpRC2[ii],Form("Bodek kin%d",ii+1),"P");
  }
  leg2->Draw();
  ...
  for(int ii=0;ii<4;ii++){
      leg3->AddEntry(gRatio[ii],Form("Bodek/gsmearing kin%d",ii+1),"P");
  }
  leg3->Draw();
