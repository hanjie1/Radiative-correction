...
void plot_H3He3()
{
  ...
  int nn=1;
  for(int ii=0;ii<11;ii++){
      if(ii<6)leg1->AddEntry(gH3HeRaw[ii],Form("H3/D kin%d",ii),"P");
      else {leg1->AddEntry(gH3HeRaw[ii],Form("H3/D kin%d",ii+nn),"P");
             nn++;
           }
  }
  leg1->Draw();
  ...
  for(int ii=0;ii<11;ii++){
      ...
      if(ii<9){
         gH3HeRC1[ii]->SetMarkerColor(ii+1);
         gH3HeRC2[ii]->SetMarkerColor(ii+1);
      }
      else {
         gH3HeRC1[ii]->SetMarkerColor(ii+2);
         gH3HeRC2[ii]->SetMarkerColor(ii+2);
      }
      mg2->Add(gH3HeRC1[ii]);
      mg2->Add(gH3HeRC2[ii]);
  }
  ...
  for(int ii=0;ii<11;ii++){
      if(ii<6){
         leg2->AddEntry(gH3HeRC1[ii],Form("gsmearing_newbin kin%d",ii),"P");
         leg2->AddEntry(gH3HeRC2[ii],Form("Bodek_newbin kin%d",ii),"P");
      }
      else{
         leg2->AddEntry(gH3HeRC1[ii],Form("gsmearing_newbin kin%d",ii+nn),"P");
         leg2->AddEntry(gH3HeRC2[ii],Form("Bodek_newbin kin%d",ii+nn),"P");
	 nn++;
      }
  }
  leg2->Draw();
