...
void plot_H3D()
{
     ...
     Double_t H3D_ratio[10][MAXBIN],H3D_err[10][MAXBIN],D2_RC1[10][MAXBIN],H3_RC1[10][MAXBIN],D2_RC2[10][MAXBIN],H3_RC2[10][MAXBIN];
  ...
  TGraphErrors *gH3DRC1[10];
  TGraphErrors *gH3DRC2[10];
  ...
  for(int ii=0;ii<10;ii++){
      ...
      for(int jj=0;jj<MAXBIN;jj++){
          ...
          gH3DRC1[ii]->SetPoint(nn1,D2_x[ii][jj],ratio1);
          gH3DRC1[ii]->SetPointError(nn1,0.0,ratio1_err);
          gH3DRC2[ii]->SetPoint(nn1,D2_x[ii][jj],ratio2);
  ...
  for(int ii=0;ii<10;ii++){
      ...
      else gH3DRaw[ii]->SetMarkerColor(46);
      mg1->Add(gH3DRaw[ii]);
  }
  mg1->Draw("AP");
  ...
  int nn=2;
  for(int ii=0;ii<10;ii++){
      if(ii<5)leg1->AddEntry(gH3DRaw[ii],Form("H3/D kin%d",ii+1),"P");
  ...
  for(int ii=0;ii<10;ii++){
      ...
      if(ii<9){
         ...
         gH3DRC1[ii]->SetMarkerColor(46);
         gH3DRC2[ii]->SetMarkerColor(46);
      }
      mg2->Add(gH3DRC1[ii]);
      mg2->Add(gH3DRC2[ii]);
  }
  ...
  for(int ii=0;ii<10;ii++){
      if(ii<5){
         leg2->AddEntry(gH3DRC1[ii],Form("gsmearing kin%d",ii+1),"P");
         leg2->AddEntry(gH3DRC2[ii],Form("Bodek kin%d",ii+1),"P");
      }
      else{
