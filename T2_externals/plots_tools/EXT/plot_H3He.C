...
void plot_H3He(){
	Double_t H3_x[MAXBIN],H3_Q2[MAXBIN],He3_x[MAXBIN],He3_Q2[MAXBIN];
	...
	for(int ii=0;ii<MAXBIN;ii++){
	    H3_x[ii]=0; H3_Q2[ii]=0; He3_x[ii]=0;  He3_Q2[ii]=0;
	...
	TString Yfile;
        ...
        ofstream outfile;
        outfile.open("OUT/H3He_RC.csv",ios::app);
        outfile<<"kin"<<kin<<endl;
        outfile<<"x,Q2,RC(all) ratio,RC(gas) ratio,RC(pre-target) ratio,RC(wall) ratio,RC(post-target) ratio"<<endl;
        for(int ii=0;ii<MAXBIN;ii++){
            if(H3_x[ii]==0||He3_x[ii]==0)continue;
	    if(abs(H3_x[ii]-He3_x[ii])>0.01)continue;
	    Double_t tmpr_RC=H3_RC[ii]/He3_RC[ii];
	    Double_t tmpr_RC_gas=H3_RC_gas[ii]/He3_RC_gas[ii];
	    Double_t tmpr_RC_pre=H3_RC_pre[ii]/He3_RC_pre[ii];
	    Double_t tmpr_RC_wall=H3_RC_wall[ii]/He3_RC_wall[ii];
	    Double_t tmpr_RC_pos=H3_RC_pos[ii]/He3_RC_pos[ii];
            outfile<<fixed;
            outfile<<setprecision(3)<<He3_x[ii]<<","<<He3_Q2[ii]<<",";
