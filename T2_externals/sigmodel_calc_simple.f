...
C       Declare locals.
	...
	logical         first
        REAL            x_save,Q2_save,ST,SLOPE,SY,DSLOPE  !variables for F2GLOB()
        real            R_save,DR,F2_save
        logical         GD
        real*8          f2n,f2nf2p
        real            f2allm
...
c	write(6,*) 'got to 1'
	...
	WSQ = -Q2 + m_p**2 + 2.0*m_p*nu 
        ...
        open(unit=44,File='OUT/integral_W.out')
        write(44,*) x,wsq,q2
...
c	   if(wsq.gt.3.0) then
	      call ineft(Q2,sqrt(wsq),W1p,W2p,dble(1.0))
	      call ineft(Q2,sqrt(wsq),W1D,W2D,dble(2.0))
c              CALL R1998(X_save,Q2_save,R_save,DR,GD)
c              call F2GLOB(x_save,Q2_save,'H',12,F2_save,ST,SY,slope,dslope,GD)
c              R=R_save
c              F2=F2_save
c              W2P=F2/NU
c              W1P=(1.0 + NU*NU/Q2)*W2P/(1.0 + R)

c              call F2GLOB(x_save,Q2_save,'D',12,F2_save,ST,SY,slope,dslope,GD)
c              F2=F2_save
c              W2D=F2/NU
c              W1D=(1.0 + NU*NU/Q2)*W2D/(1.0 + R)

	      W1n=2.0*W1D-W1p
	      W2n=2.0*W2D-W2p

c              F2_save=F2ALLM(x_save,Q2_save)    !proton F2
c              call R1990F(x_save,Q2_save,R_save,DR)
c              R=R_save
c              F2=F2_save
c              W2P=F2/NU
c              W1P=(1.0 + NU*NU/Q2)*W2P/(1.0 + R)
